...
class Embedding(nn.Module):
    ...
    def forward(self, x, emb):
        return x + self.emb_layer(emb)
        
class PanModel(nn.Module):

    def __init__(self, in_nc=3, nb=16, nf=40, out_nc=3, unf=24, bam=True, scale=2):
        super().__init__()
        self.scale = scale
        # SCPA               
    ...
    def forward(self, x):
        ...
        if self.scale in (1, 2, 3):
            if self.scale >  1:
                fea = F.interpolate(fea, scale_factor=self.scale, mode='nearest')
